...
from kothic.backend.postgis import PostGisBackend as DataBackend
#from kothic.backend.vtile import QuadTileBackend as DataBackend
from kothic.mapcss import MapCSS as Styling
from kothic.gtk_widget import KothicWidget
from addr_selector.gtk_widget import AddrSelector
...
class KothicApp:
  def __init__(self):
    self.width, self.height = 1024, 740
    
    self.center_coord = (27.6549791, 53.8698)
    self.zoom = 17.
    self.data_projection = "EPSG:4326"
    self.data = DataBackend(path="data/tiles")
    self.load_style()
    ...
    self.AddrSelector = AddrSelector()
    self.AddrSelector.connect("box_changed", self.printa)
    hbox = gtk.HBox()
    
    hbox.pack_start(self.KothicWidget)
    hbox.pack_end(self.AddrSelector, False, False, 0)
    vbox.pack_end(hbox)
  ...
  def printa(self, widget):
    print widget.bbox
    self.KothicWidget.zoom_to(widget.bbox)
  def load_style(self):
